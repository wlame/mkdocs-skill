...
Read `${CLAUDE_PLUGIN_ROOT}/skills/ghdoc/references/repo-analysis.md` and execute it exactly:
launch the 4 Explore agents in parallel and save the findings to `.claude/ghdoc-analysis.md` in
the documented format. (The procedure is shared with `/ghdoc-update` — do not restate or vary the
agent prompts here.)

The concept inventory and existing-docs sections of the analysis file are both critical — they are
passed to content agents in Phase 4 and the cross-linking agent in Phase 5.
